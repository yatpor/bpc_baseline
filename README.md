...
| Concept | Explanation |
|---------|------------|
| **Intrinsic Camera Matrix (`cam_K`)** | A **3×3 matrix** that defines the camera’s internal parameters, including focal length and optical center. It is used to project **3D points onto a 2D image plane**. The general form is: <br> `K = [[f_x, 0, c_x], [0, f_y, c_y], [0, 0, 1]]` <br> where `(f_x, f_y)` are the focal lengths and `(c_x, c_y)` are the optical center coordinates. |
...
| **Translation Vector (`cam_t_w2c`, `cam_t_m2c`)** | A **3×1 vector** that defines the translation (shift) of a coordinate system. It moves points from one coordinate system to another. The full transformation from world to image coordinates is: <br> `P_i = K * [R | t] * P_w` <br> where `P_w` is a **3D point in world coordinates**, and `P_i` is its **2D projection in image coordinates**. |
...
| **Visibility Fraction (`visib_fract`)** | A value between **0 and 1**, representing how much of the object is visible in the image. Computed as: <br> `visib_fract = px_count_visib / px_count_all` <br> where `px_count_visib` is the number of visible pixels and `px_count_all` is the total object pixel count. |
